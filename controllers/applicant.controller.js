  ...
  async (req, res, next) => {
    try {
      ...
      res.status(200).json({
        ...
        data: newProfile ? newProfile.resumeFiles : applicantProfile.resumeFiles,  // Trả về thông tin file đã tải lên
      });
    } catch (error) {
      next(error);
    }
...
export const deleteUploadedCV = async (req, res, next) => {
  try {
    ...
    const cvToDelete = applicantProfile.resumeFiles.find(cv => cv._id.toString() === cvId);
    if (cvToDelete) {
    ...
    applicantProfile.resumeFiles = applicantProfile.resumeFiles.filter(cv => cv._id.toString() !== cvId);
